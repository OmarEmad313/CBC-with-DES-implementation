...
def permute(k, arr, n):  # k=key , arr= permutation array of length 56 or 48 or 32, n=56 or 48
    permutation = ""
    for i in range(0, n):
...
# dividing user input into blocks of 64 bits
def inputToBlocks(userInput2bin):
    times = 0
    list = []
    init = 0
    tempstring = ''
    for i in range(1, len(userInput2bin) + 1):
        if i % 64 == 0:
            # print('in if', i)
            times += 1
            for k in range(init, i):
                tempstring += userInput2bin[k]
                init = k + 1
...
# Initial Permutation Table of 64 bits, to permute the plain text
...
# Expansion D-box Table : to transfer the plaint text from 32 to 48 bits (bec key is = 48 bits)
...
# print('key length is ', len(key), 'bits')
...
ct = '4D60E325F9573C0B211A94B9586C0E8F11DF156FD3C0221C'
ct2bin = hex2bin(ct)
...
class MyWindow:
    ...
    def add(self):
        ...
        userInput =self.t1.get()
        userInput2bin = ''.join(format(ord(i), '08b') for i in userInput)
        myList = inputToBlocks(userInput2bin)
        final_ciphertext = encrypt_CBC(myList, iv)
        self.t3.insert(END,bin2hex(final_ciphertext) )
    def sub(self, event):
        self.t3.delete(0, 'end')
        ct =self.t1.get()
        ct2bin = hex2bin(ct)
